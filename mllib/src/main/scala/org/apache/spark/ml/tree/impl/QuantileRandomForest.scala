...
private[spark] object QuantileRandomForest extends Logging with Serializable {
  ...
  def run(
     ...
     */
    val nodeStack = new mutable.ArrayStack[(Int, LearningNodeV2)]

    val rng = new Random()
    rng.setSeed(seed)

    // Allocate and queue root nodes.
    val topNodes = Array.fill[LearningNodeV2](numTrees)(LearningNodeV2.emptyNode(nodeIndex = 1))
    ...
    while (nodeStack.nonEmpty) {
      ...
      val (nodesForGroup, treeToNodeToIndexInfo) =
        QuantileRandomForest.selectNodesToSplit(nodeStack, maxMemoryUsage, metadata, rng)
      // Sanity check (should never occur):
      assert(nodesForGroup.nonEmpty,
        s"QuantileRandomForest selected empty nodesForGroup.  Error for unknown reason.")

      // Only send trees to worker if they contain nodes being split this iteration.
      val topNodesForGroup: Map[Int, LearningNodeV2] =
        nodesForGroup.keys.map(treeIdx => treeIdx -> topNodes(treeIdx)).toMap

      // Choose node splits, and enqueue new nodes as needed.
      timer.start("findBestSplits")
      QuantileRandomForest.findBestSplits(baggedInput, metadata, topNodesForGroup, nodesForGroup,
        treeToNodeToIndexInfo, splits, nodeStack, timer, nodeIdCache)
      timer.stop("findBestSplits")
    }

    populateLabelsInLeaves(baggedInput, topNodes, splits)
  ...
  class TreeNodeLabel(val treeIdx: Short, val nodeIdx: Int, val label: Float) extends Serializable

  def populateLabelsInLeaves(baggedInput: RDD[BaggedPoint[TreePoint]], topNodes: Array[LearningNodeV2], splits: Array[Array[Split]]) : Unit = {
    val treeNodeLabels = baggedInput.mapPartitions {
      points => {
        val result = mutable.ArrayBuffer[TreeNodeLabel]()
        for (point <- points) {
          val pointLabel = point.datum.label.toFloat
          for ((topNode,treeIdx) <- topNodes.zipWithIndex ) {
            val populationInTree = (point.subsampleWeights(treeIdx) + 0.5).toInt
            if (populationInTree > 0) {
              val leafIdx = topNode.predictImpl(point.datum.binnedFeatures, splits)
              for (i <- 1 to populationInTree) {
                result += (new TreeNodeLabel(treeIdx.toShort, leafIdx, pointLabel))
              }
            }
          }
        }
      ...
      .collect
      .view
      .foreach(treeNodeLabel => {
        val treeRoot = topNodes(treeNodeLabel.treeIdx)
        val leaf = LearningNodeV2.getNode(treeNodeLabel.nodeIdx, treeRoot)
        if (leaf.labels == None) {
          leaf.labels = Some(mutable.ArrayBuffer[Float]())
        }
        leaf.labels.get += treeNodeLabel.label
      })
  }
  ...
  private[tree] def findBestSplits(
      ...
      topNodesForGroup: Map[Int, LearningNodeV2],
      nodesForGroup: Map[Int, Array[LearningNodeV2]],
      treeToNodeToIndexInfo: Map[Int, Map[Int, NodeIndexInfo]],
      splits: Array[Array[Split]],
      nodeStack: mutable.ArrayStack[(Int, LearningNodeV2)],
    ...
    val nodes = new Array[LearningNodeV2](numNodes)
    ...
    nodesForGroup.foreach { case (treeIndex, nodesForTree) =>
      nodesForTree.foreach { node =>
        ...
        val isLeaf =
          (stats.gain <= 0) || (LearningNodeV2.indexToLevel(nodeIndex) == metadata.maxDepth)
        node.isLeaf = isLeaf
        node.stats = stats
        logDebug("Node = " + node)
        ...
        if (!isLeaf) {
          ...
          val childIsLeaf = (LearningNodeV2.indexToLevel(nodeIndex) + 1) == metadata.maxDepth
          val leftChildIsLeaf = childIsLeaf || (stats.leftImpurity == 0.0)
          val rightChildIsLeaf = childIsLeaf || (stats.rightImpurity == 0.0)
          node.leftChild = Some(LearningNodeV2(LearningNodeV2.leftChildIndex(nodeIndex),
            leftChildIsLeaf, ImpurityStats.getEmptyImpurityStats(stats.leftImpurityCalculator)))
          node.rightChild = Some(LearningNodeV2(LearningNodeV2.rightChildIndex(nodeIndex),
  ...
  private[tree] def binsToBestSplit(
      ...
      node: LearningNodeV2): (Split, ImpurityStats) = {

    // Calculate InformationGain and ImpurityStats if current node is top node
    val level = LearningNodeV2.indexToLevel(node.id)
  ...
  private[tree] def selectNodesToSplit(
      ...
      maxMemoryUsage: Long,
      metadata: DecisionTreeMetadata,
      rng: Random): (Map[Int, Array[LearningNodeV2]], Map[Int, Map[Int, NodeIndexInfo]]) = {
    // Collect some nodes to split:
    //  nodesForGroup(treeIndex) = nodes to split
    val mutableNodesForGroup = new mutable.HashMap[Int, mutable.ArrayBuffer[LearningNodeV2]]()
    ...
    while (nodeStack.nonEmpty && !groupDone) {
      ...
      val nodeMemUsage = QuantileRandomForest.aggregateSizeForNode(metadata, featureSubset) * 8L
      if (memUsage + nodeMemUsage <= maxMemoryUsage || memUsage == 0) {
        nodeStack.pop()
        mutableNodesForGroup.getOrElseUpdate(treeIndex, new mutable.ArrayBuffer[LearningNodeV2]()) +=
          node
    ...
    val nodesForGroup: Map[Int, Array[LearningNodeV2]] =
